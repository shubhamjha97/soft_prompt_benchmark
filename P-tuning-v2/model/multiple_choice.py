...
from transformers import BertModel, BertPreTrainedModel, GPT2PreTrainedModel, GPT2Model
...
class GPT2PrefixForMultipleChoice(GPT2PreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"position_ids"]
    ...
    def forward(
        ...
        past_key_values = self.get_prompt(batch_size=batch_size * num_choices)
        prefix_attention_mask = torch.ones(batch_size * num_choices, self.pre_seq_len).to(self.gpt2.device)
        flat_attention_mask = torch.cat((prefix_attention_mask, flat_attention_mask), dim=1)

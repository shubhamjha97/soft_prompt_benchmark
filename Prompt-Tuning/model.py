...
from transformers import GPT2LMHeadModel, GPTNeoForCausalLM, T5ForConditionalGeneration, RobertaForCausalLM
import torch
import torch.nn as nn

class BasePromptTuningMixin:

    def _get_embedding_layer(self):
        raise NotImplementedError

    def _set_embedding_layer(self):
        raise NotImplementedError
    ...
    def from_pretrained(
        ...
        model._initialize_embedding_size()
    ...
    def _cat_learned_embedding_to_input(self, input_ids) -> torch.Tensor:
        inputs_embeds = self._get_embedding_layer()(input_ids)
...
class T5PromptTuningMixin(BasePromptTuningMixin):
    def _get_embedding_layer(self):
        return self.shared
...
class RobertaPromptTuningLM(RobertaPromptTuningMixin, RobertaForCausalLM):
    def __init__(self, config):
        super().__init__(config)
